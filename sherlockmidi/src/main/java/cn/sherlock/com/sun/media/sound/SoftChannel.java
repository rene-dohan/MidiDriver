...
public class SoftChannel implements MidiChannel, ModelDirectedPlayer {
        ...
        synchronized (control_mutex) {
            ...
            for (SoftVoice voice : voices) {
                ...
                if (voice.stealer_channel == this && voice.stealer_noteNumber == noteNumber) {
                    ...
                    voice.stealer_channelmixer = null;
                }
            }
    ...
    public void setChannelPressure(int pressure) {        
        ...
        synchronized (control_mutex) {
            ...
            for (SoftVoice voice : voices) {
                if (voice.active)
                    voice.setChannelPressure(pressure);
            }
        }
    }
    ...
    protected void applyInstrumentCustomization() {
        ...
        for (int i = 0; i < new_performers.length; i++) {
            ...
            if (cds_polypressure_connections != null) {
                ...
                connblocks.addAll(Arrays.asList(cds_polypressure_connections));
            }
            ...
            if (cds_channelpressure_connections != null) {
                ...
                Collections.addAll(connblocks, cds_channelpressure_connections);
            }
    ...
    private ModelConnectionBlock[] createModelConnections(ModelIdentifier sid,
        ...
        */
        ...
        return conns.toArray(new ModelConnectionBlock[0]);
    }
...
*/
        ...
        if (controller < 120) {
            for (SoftVoice voice : voices)
                if (voice.active)
                    voice.controlChange(controller, -1);
    ...
    public void controlChange(int controller, int value) {
        ...
        synchronized (control_mutex) {
            ...
            for (SoftVoice voice : voices)
                if (voice.active)
                    voice.controlChange(controller, value);
    ...
    public void nrpnChange(int controller, int value) {
        ...
        for (SoftVoice voice : voices)
            if (voice.active)
                voice.nrpnChange(controller, val_i[0]);
    ...
    public void rpnChange(int controller, int value) {
        ...
        for (SoftVoice voice : voices)
            if (voice.active)
                voice.rpnChange(controller, val_i[0]);
    }
